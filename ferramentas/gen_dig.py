...
memorias nativas (ROM / RAMDualPort / Splitter) seguem o layout de pinos do
Digital para esses componentes; as do bloco VHDL (ExternalFile) seguem a regra:
...
def build(rom_data, code_file="riscv_cpu.vhd", ghdl_workdir=None):
    ...
    # Pinos (layout Digital): A@(Rx,Ry)  sel@(Rx,Ry+40)  D@(Rx+60,Ry+20)
    Rx, Ry = 640, -120
    ...
    # Pinos (layout Digital): ADDR@(Mx,My) Din@(My+20) we@(My+40) C@(My+60)
