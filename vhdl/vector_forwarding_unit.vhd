...
end vector_forwarding_unit;

architecture Behavioral of vector_forwarding_unit is
begin
    process(ID_EX_Rs1_i, ID_EX_Rs2_i, EX_MEM_Rd_i, MEM_WB_Rd_i, EX_MEM_RegWrite_i, MEM_WB_RegWrite_i)
    begin
        -- Estado normal, pega do estágio atual
        ForwardA_o <= "00";
        ForwardB_o <= "00";

        -- Forwarding no operando A vetorial
        if (EX_MEM_RegWrite_i = '1' and EX_MEM_Rd_i = ID_EX_Rs1_i) then
            ForwardA_o <= "10";
        elsif (MEM_WB_RegWrite_i = '1' and MEM_WB_Rd_i = ID_EX_Rs1_i) then
            ForwardA_o <= "01";
        end if;

        -- Forwarding no operando B vetorial
        if (EX_MEM_RegWrite_i = '1' and EX_MEM_Rd_i = ID_EX_Rs2_i) then
            ForwardB_o <= "10";
        elsif (MEM_WB_RegWrite_i = '1' and MEM_WB_Rd_i = ID_EX_Rs2_i) then
            ForwardB_o <= "01";
        end if;
    end process;
end Behavioral;